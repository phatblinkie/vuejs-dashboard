...
export default {
  ...
  methods: {
    async fetchHosts() {
      try {
        const resp = await axios.get(
          'http://192.168.11.165/post/get_patch_status_hosts_sqlite.php?project_id=2',
        );
    ...
    async fetchHostDetails(hostname, osType) {
      try {
        const resp = await axios.get(
          `http://192.168.11.165/post/get_combined_patch_status_sqlite.php?project_id=2&hostname=${hostname}&os_type=${osType}`,
        );
